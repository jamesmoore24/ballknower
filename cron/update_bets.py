...
def update_odds():
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE bets SET odds = ? WHERE team = ?", (random.uniform(1.0, 3.0), 'Team A'))
    conn.commit()
    conn.close()
    print("Updated odds for Team A")
...
if __name__ == "__main__":
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("UPDATE bets SET odds = ? WHERE team = ?", (scraped_odds, 'Team A'))
    conn.commit()
    conn.close()
