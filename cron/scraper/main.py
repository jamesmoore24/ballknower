...
def main():
    """
    Entry point for command-line usage. Parses arguments and calls the appropriate function.
    """
    parser = argparse.ArgumentParser(
        description="Process traffic.mitm file(s) and route requests to appropriate parsers."
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--file", "-f",
        help="Path to a single traffic.mitm file to process"
    )
    group.add_argument(
        "--dir", "-d",
        help="Path to a directory containing .mitm files"
    )
    ...
    if args.file:
        ...
        process_traffic_file(args.file)
    elif args.dir:
        logger.info(f"Processing all .mitm files in directory: {args.dir}")
